# -*- coding: utf-8 -*-
"""
Created on Mon Jun 16 12:31:55 2025
...
def prop_eval (x,y,t1,t2):
    l = []
    for i in range(len(x)):
        l.append(act_identifier(x[i],y[i],t1,t2))
    l = np.array(l)
    return l

#Funzione che identifica l'attività
def act_identifier (h_AP, CC_max, t1, t2):
...
def best_thresholds_calc(properties):
    ...
    for t1 in thresholds_range["threshold_1"]:
        for t2 in thresholds_range["threshold_2"]:
            thresholds = [t1,t2]
            l = np.repeat(prop_eval(properties[:,0],properties[:,1],t1,t2),24)
            
            conf_matrx = np.round(confusioner(y[0:len(l)],l)*100).astype(int)
            accuracy = np.mean([conf_matrx[0,0],conf_matrx[1,1],conf_matrx[2,2]])
            
            if accuracy > best_accuracy and conf_matrx[0,0]>0.5 and conf_matrx[1,1]>0.5 and conf_matrx[2,2]>0.5:
                best_accuracy = accuracy
                best_thresholds = thresholds
    return best_thresholds, best_accuracy
 
def database_test(s,w,j):
    ...
    data = np.vstack((stop, walk, jog))
    steps = [steps_w, steps_j]
    lens = [len_w, len_j]
    return data, steps, lens
            
def plot_conf(conf_mtrx,title):
    plt.figure(figsize=(8, 6))
    sns.heatmap(conf_mtrx, annot=True, fmt='g', cmap='GnBu', xticklabels=['Stop', 'Walk', 'Jog'], yticklabels=['Stop', 'Walk', 'Jog'])
...
base_dir, Dir , Test, Train = loader('009')
...
for i in range(50):
    ...
    test,steps,lens = database_test(s_,w_,j_)
    AP = test[:,0]
    CC = test[:,1]
    ML = test[:,2]
    Y = test[:,3]
    ...
    t.append([T1, T2, CW, CJ])
    
    l = np.repeat(prop_eval(prop_test[:,0],prop_test[:,1],T1,T2),24)
    y_temp = Y[:len(l)]
    f = flags[:len(l)]
...
t = np.vstack(t)
plt.figure(figsize=(8, 6))
plt.boxplot([t[:,0], t[:,1]], tick_labels=['t1 (Stop-Walk)', 't2 (Walk-Jog)'])
plt.title('Boxplot of Threshold Arrays')
plt.ylabel('Value')
plt.grid(True)
plt.show()
...
plt.figure(figsize=(8, 6))
plt.boxplot([t[:,2], t[:,3]], tick_labels=['Cw', 'Cj'])
